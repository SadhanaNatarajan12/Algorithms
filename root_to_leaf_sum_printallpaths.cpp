...
void root_to_leaf_sumHelper(Node *root, int K, std::vector<int> &vec, int sum)
{
    ...
    vec.pop_back(); //remove the last element from the vector and move to the parent
}

void root_to_leaf_all_paths_sum(Node *root, int K)
{
    std::vector<int> vec;
    int sum = 0;
...
int main()
{
    ...
    root_to_leaf_all_paths_sum(root, 26);
